...
def run_bench(
    ...
    embed = torch.randn(V, D, device=device, dtype=torch.float32)
    hidden = torch.randn(N, D, device=device, dtype=torch.float32)  # 相当于 s_transT
    t_flat = torch.randn(N, D, device=device, dtype=torch.float32)  # 教师侧隐状态，用于生成 topk
    ...
    with torch.no_grad():
        for i in range(0, N, chunk_size):
            ...
            t_logits = t_chunk @ embed.T
            t_topk_vals, t_topk_indices = torch.topk(t_logits, k=topk, dim=-1)
